...
import 'package:pharmacy_app/features/customer_return_invoices/cubit/customer_return_invoice_detail_cubit.dart';
...
/// Read-only detail for a customer return invoice: header (number, date,
/// customer, refund method, status), totals card (subtotal, tax, discount,
/// refund total), the returned items, notes, and a Cancel action (with confirm
/// dialog). Mirrors [SalesInvoiceDetailScreen].
class CustomerReturnInvoiceDetailScreen extends StatefulWidget {
  const CustomerReturnInvoiceDetailScreen({super.key, required this.invoiceId});

  final int invoiceId;

  @override
  State<CustomerReturnInvoiceDetailScreen> createState() =>
      _CustomerReturnInvoiceDetailScreenState();
}

class _CustomerReturnInvoiceDetailScreenState
    extends State<CustomerReturnInvoiceDetailScreen> {
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => CustomerReturnInvoiceDetailCubit(
...
class _Body extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tr = context.tr;
    final cubit = context.read<CustomerReturnInvoiceDetailCubit>();

    return BlocListener<CustomerReturnInvoiceDetailCubit,
        CustomerReturnInvoiceDetailState>(
      listenWhen: (p, c) =>
          p.lastAction != c.lastAction || p.failure != c.failure,
      listener: (context, state) {
        if (state.lastAction == CustomerReturnDetailAction.cancelled) {
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: BlocBuilder<CustomerReturnInvoiceDetailCubit,
              CustomerReturnInvoiceDetailState>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: Text(tr.return_invoice_cancelled),
          ),
        ],
      ),
    );
        ...
        children: [
          ...
          if (invoice.customer != null) ...[
            ...
            _MetaRow(
              ...
              text:
                  '${tr.sales_invoice_customer}: ${invoice.customer!.fullName}',
            ),
          ],
          if (invoice.originalSalesInvoiceId != null) ...[
            context.vXs,
            _MetaRow(
              icon: Icons.receipt_long_outlined,
              text: '${tr.return_invoice_original}: #${invoice.originalSalesInvoiceId}',
            ),
          ],
          context.vXs,
          _MetaRow(
          ...
          if (invoice.reason != null && invoice.reason!.isNotEmpty) ...[
            ...
            _MetaRow(
              ...
              text: '${tr.return_invoice_reason}: ${invoice.reason}',
            ),
          ],
        ],
      ),

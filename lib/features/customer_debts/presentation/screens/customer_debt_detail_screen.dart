...
import 'package:pharmacy_app/features/customer_debts/cubit/customer_debt_detail_cubit.dart';
...
/// Read-only detail for a single customer debt: summary card + amounts + the
/// payments timeline + a Record Payment action (with a confirm sheet).
class CustomerDebtDetailScreen extends StatelessWidget {
  const CustomerDebtDetailScreen({super.key, required this.debtId});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          CustomerDebtDetailCubit(repository: sl<CustomerDebtRepository>())
            ..loadDebt(debtId),
...
class _Body extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tr = context.tr;
    final cubit = context.read<CustomerDebtDetailCubit>();

    return BlocListener<CustomerDebtDetailCubit, CustomerDebtDetailState>(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: BlocBuilder<CustomerDebtDetailCubit, CustomerDebtDetailState>(
            builder: (context, state) {
              ...
              if (state.failure != null && state.debt == null) {
                return _Error(
                  message: state.failure!.localizedMessage(context),
                  onRetry: () =>
                      context.read<CustomerDebtDetailCubit>().loadDebt(debtId),
                );
              }
              if (state.debt == null) {
                return _Loading();
              ...
              return RefreshIndicator(
                onRefresh: () =>
                    context.read<CustomerDebtDetailCubit>().refresh(debtId),
                child: ListView(
...
class _PayAction extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final canPay =
        debt.status != CustomerDebtStatus.paid &&
        debt.status != CustomerDebtStatus.cancelled &&
